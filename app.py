...
class ExecutiveEngineApp:
    def __init__(self):
        ...
        # --- The CFO Module (Option A Database Sync) ---
        # 1. Ask the database what mode we should be in
        self.LIVE_MODE = (self._get_system_mode() == 'LIVE')

        # 2. Initialize the Treasury based on that mode
        if self.LIVE_MODE:
            ...
            print("📝 Booting in PAPER TRADING mode...")
    ...
    def _set_system_mode(self, mode_string):
        ...
        try:
            conn = self.get_db_connection()
            cur = conn.cursor()
            cur.execute("INSERT INTO system_config (key, value) VALUES ('trading_mode', %s) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value;", (mode_string,))
            conn.commit()
            cur.close()
            conn.close()
        except Exception as e:
    ...
    def _setup_routes(self):
        ...
        def toggle_mode():
            ...
            # FIXED: Tell the database so the scanner.py background process knows!
            env_str = "LIVE" if self.LIVE_MODE else "PAPER"
            self._set_system_mode(env_str)
        ...
        def override_open():
            ...
            import json
            data = request.json
            sym = data.get('symbol') # e.g., "BTC/USD"
            env_str = "LIVE" if self.LIVE_MODE else "PAPER"

            # FIXED: Normalize ID using the factory rule (maps BTC -> xbt_pure)
            ...
            try:
                ...
                # FIXED: Instantiate strategy to use unified bracket math rules
                playbook = get_strategy("sniper_v1")
                max_tranches = playbook.max_tranches
                tranche_usd = total_allocated / max_tranches
                qty = tranche_usd / price
                
                # Pull formulas directly from the strategy playbook
                brackets = playbook.calculate_brackets(price, atr_pct)
